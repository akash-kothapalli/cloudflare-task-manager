...
export async function findAll(db: D1Database, userId: number, params: TaskQueryParams): Promise<FindAllResult> {
	...
	const tasksWithTags = await Promise.all(tasks.map((task) => toTaskResponse(db, task)));
...
export async function findById(db: D1Database, id: number, userId: number): Promise<TaskResponse | null> {
	const row = await db.prepare('SELECT * FROM tasks WHERE id = ? AND user_id = ?').bind(id, userId).first<Record<string, unknown>>();
...
export async function create(db: D1Database, userId: number, input: CreateTaskInput): Promise<TaskResponse> {
    ...
    `,
		...
		.bind(userId, input.title, input.description ?? null, input.status ?? 'todo', input.priority ?? 'medium', input.due_date ?? null)
		.run();
...
export async function updateAiFields(db: D1Database, id: number, userId: number, aiSummary: string, aiSentiment: string): Promise<void> {
	await db
		.prepare(
			`
      UPDATE tasks
