...
export async function createUser(db: D1Database, email: string, name: string, password: string): Promise<User> {
	await db.prepare('INSERT INTO users (email, name, password) VALUES (?, ?, ?)').bind(email, name, password).run();
	...
	const row = await db.prepare('SELECT * FROM users WHERE id = last_insert_rowid()').first<Record<string, unknown>>();
...
export async function updateUser(db: D1Database, id: number, name?: string): Promise<User | null> {
	await db
